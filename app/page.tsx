...
export default function Page(){
 ...
 const [demoOpen,setDemoOpen]=useState(false); const [demoText,setDemoText]=useState(demoToText(demo)); const [demoMessage,setDemoMessage]=useState('');
 ...
 const visibleInputFunds=showAllInput?funds:funds.slice(0,5);
 const visibleRows=showAllRows?sortedRows:sortedRows.slice(0,6);
 ...
 return <main>
  {showDisclaimer&&<div className="modalMask"><div className="modal"><h2>免责声明</h2><p>FundCare 仅用于基金组合信息整理、持仓结构分析、新闻情绪辅助判断和 AI 文本生成展示。本工具不提供确定性收益预测，不构成任何基金、证券或其他金融产品的买入、卖出、持有建议。</p><p>页面中的涨跌幅、预估盈亏、新闻情绪和 AI 报告均可能存在延迟、误差或不完整情况。用户应以基金公司、交易平台及监管机构披露的信息为准，并自行承担投资决策风险。</p><div className="modalActions"><a href="/disclaimer" target="_blank">查看完整免责声明</a><a href="/privacy" target="_blank">隐私政策</a><button className="primary" onClick={()=>setShowDisclaimer(false)}>我已知晓并继续使用</button></div></div></div>}
